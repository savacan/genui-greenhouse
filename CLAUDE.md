...
- 2026-06-26: **exp03 pokefinder 着手（`exp/03-pokefinder`）= GenUI の役割反転（出力描画 → 双方向の入力UIを組む）**。動かす唯一の変数＝「LLM が `$bindState` で two-way 入力フォームを組めるか」。設計は [`docs/pokefinder.md`](docs/pokefinder.md) が正（カクテル廃案 → PokéAPI ピボットの経緯含む）。**Phase A 完了**（データ層 `lib/finder` ＋ probe で積集合チェーンを実 PokéAPI 検証・§8）。**Phase B 完了**（レンダラ写経＋two-way 部品＋ハンド spec を :3103 でブラウザ検証・§9）。要点: store は `initialState` から seed（`spec.state` ではない＝Phase C は LLM の spec.state を抽出して渡す）／`$template` は core binding 式・トークン `${/ptr}`／「探す」は find ハンドラ＋onStateChange 再構成 ref で閉じる（action params に state 解決させない）。**NEXT = Phase C**（単発 compose で LLM にフォームを組ませ、組成品質をワークフローで多サンプル評定）。
