...
- [ ] pnpm workspace + `experiments/` 骨格
- [ ] **単発オーケストレーション**を1本通す: ユーザー入力 → LLM がアクション選択 → サーバが実データ取得＆**計算** → spec → 描画

### Phase 1 — オーケストレーションを段階的に

- [ ] マルチターン（直前の文脈を踏まえて UI を更新）
- [ ] 操作のフィードバックループ（クリック / フィルタ → 再取得。json-render の state / actions / watch）
- [ ] ストリーミング描画（spec を逐次）
- [ ] 2nd pass（取得データに所見を書く等の追い足し）

### Phase 2 — 横に広げて学ぶ

- [ ] 別の型 / 題材でもう1本（GenUI が**効く / 効かない境界**を体感）
