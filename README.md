...
| 03 | [pokefinder](experiments/03-pokefinder/) | **入力UI生成（役割反転）** × ポケモン | GenUI が**双方向の入力フォーム**を組む（出力描画 → 入力/state）。[docs §8–§16](docs/pokefinder.md)。LLM の `$bindState` 組成は高品質（fit/intent 4.45/5・契約遵守は完璧）、境界は **client の state/handler ライフサイクル**（Compose-Live × handler 凍結）・**データ形**（フォーム重複）・**サーバが表現できない意図**（OR/範囲/情緒語を黙って近似＝サイレント縮約 → 中立＋明示で“透明な劣化”へ）。controlled store で remount-flash を解消。§14＝**指差しで組み直す**（出力カード → LLM が「似た相棒」フォームを再 compose）。§14b＝**線を太くして忠実化**（findMons に type OR・世代範囲を足し OR→AND サイレント縮約を源から解消）。**ただし途中で実バグ**：候補をアルファベット順に60件で切ってからランクしていて「最強の◯◯」が誤答（reshiram/rayquaza が落ちる）→ 全候補をランクする修正。§15＝**がっつり監査**（8サーフェスを実 PokéAPI と数値突き合わせ）で**違和感の正体＝フォーム汚染**を特定（type/&lt;t&gt; がメガ/キョダイ/eternamax を全部含み異形が結果を占有）→ 既定 base 種のみ＋「別形態も含める」トグル・世代フィルタ粒度差バグ（giratina 消滅）・sortBy 未配線も修正。教訓＝**「開示」は「正解」の代わりにならない／計算が健全でも“出てきたものが妥当か”を見ないと破綻は出ない（人が触って発覚）／破綻はデータの粒度という設計判断**。§16＝**エンドツーエンド意味検証**（クエリ→フォーム→検索→「返ってきた相棒が問い通りか」を ground truth で14クエリ判定）で「出力の妥当性を見る」を反復可能な測定にし、構造化監査が見逃した blocker（404 で検索全体クラッシュ）を一発で検出。**核「LLM は双方向フォームを組める」は YES 確定（OR→AND 反転も克服）・残る破綻は全てサーバ側**。発見→修正→再測＋多サンプル評定＋ground-truth 監査＋E2E 意味検証（ワークフロー）。 | PR #3 マージ済 |
| 04 | [artfinder](experiments/04-artfinder/) | **入力UI生成（役割反転）** × アート（Art Institute of Chicago） | 役割反転を**別題材でもう1本**（[docs §0–§13](docs/artfinder.md)）。pokefinder と同じ finder アーキタイプ・同じ入力役割のまま題材を AIC に替え、**§16 の結論「伸びしろはサーバ側」が別ドメイン・別クエリ形でも立つか**を検証。pokefinder と違い**上流（AIC）が豊かな ES クエリ DSL を持つ**ので「線の幅」は自前計算でなく**サーバの form→ES 翻訳忠実度＋上流の射程**に移る。**新モダリティ＝色相スウォッチ・自由テキスト・年代レンジも two-way で正しく組めた**。E2E 多レンズ判定（13クエリ・発見→修正→再測の閉ループ）で **fidelity 4.5・忠実 12/13・核は YES**（OR→AND 反転も再発せず）。破綻は一貫して **server/data 側**（クエリ言語の表現力＝版画の地域/主題/価格/OR・データ粒度＝版画は別部門・date_start=0 欠損）。閉ループの教訓＝**プロンプト nudge は単調改善でない**（狙った3破綻を直すと別1件が回帰＝「表現不能語の中立化」と「表現可能語の符号化」の境界に run 変動）。**データ源クセ（§16級）＝AIC 画像は cross-origin 表示不可**（Cloudflare＋CORP・実ブラウザ検証で判明）→色チップ視覚化に転化。exp02「線の幅↔射程」/§15「粒度はサーバ」が別ドメインで再確認。**§13 で実際に線を太く**（subject 主題・region 産地＋大陸展開・クロスファセット OR `combineMode` をサーバに追加）し再測＝**fitAvg 3.9→4.6・忠実 17/17**（線拡張は単調に fit を上げる＝thesis を数値で実証）。残課題＝主題の relevance ランキング（集合は正しいが top-N の並びが弱い・server）／プロンプト nudge の非単調性（#5↔#10 の type 取りこぼし whack-a-mole・llm）。 | `exp/04-artfinder` |
...
- [x] 別の型 / 題材でもう1本（GenUI が**効く / 効かない境界**を体感）（→ exp02 = エージェント結果可視化 × 地球モニタ／exp03 = 入力UI生成 × ポケモン／exp04 = 入力UI生成 × アート＝役割反転を別題材で再検証）
