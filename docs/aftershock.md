...
## 11. 「濃すぎ」側を叩く — トップN拡大の限界（2026-06-25 ライブ・Azure OpenAI・計測込み）

§8 で「薄すぎ→thrash／適切に広げると収束＋安い」を示したが、**広げ続けると今度はどこで「濃すぎ」に転ぶか**は未確認だった。`topEvents` の幅（`ADDRESSABLE_TOP_N`）を変えて多エンティティを増やし、境界を叩いた。

### 観察（ライブ）
- **トップ8・cap=5**: モデルは **5件だけ**ドリルして done（quakes→quakeDetail×5→weather×5,nearby×5→done・5手）。**thrash しない**（quakes 撃ち直しゼロ）が、**#6–8 を静かに切り捨てた**（addressable list に無いから名指せない）。totalIn=11.0k。盤面は綺麗な5件比較。
- **トップ10・cap=10**: モデルは **10件全部**ドリル（step2 で quakeDetail×10 を並列、step3 で **weather×10＋nearby×10＝20 tool calls を1手で並列**、step4 done・**4手**）。盤面は **208要素**（AlertBanner×10/WeatherTile×10/Sparkline×10/ShakeMap×8/Beachball×5/Kpi×41…）で**全10件が coherent に組成**。

### 地図エントリ
- **射程は「線の幅」に正比例**: cap=5 だと「トップ8」は静かに5件に truncate、cap=10 だと10件出る。`ADDRESSABLE_TOP_N` がモデルの多エンティティ射程の上限つまみ。**cap を超える要求は「静かな過少提供」**（thrash でなく silent truncation・UX 上は「8件頼んだのに5件」と気づけない方が危ない）。
