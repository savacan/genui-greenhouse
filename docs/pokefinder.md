...
**検証**: 「炎か飛行で素早さ高め」→ typeMode Select=**OR**・fire/flying ON・speed100＋「OR で初期化」明示 → 探す → 14件に **single-fire**(charizard-mega-x 等) と **single-flying**(aerodactyl 等) が両方入り和集合を実証（旧 AND では出ない）。**§13 最大破綻だった OR→AND サイレント意図反転が“源から”消えた**。AND/OR 判別を型数・接続詞を変えた8サンプルで測り **`overAppliedOr=false`**（OR は OR 文脈のみ・単一タイプは Select 省略 and 既定・曖昧な「炎と飛行」は and＋両読み明示・範囲も忠実・発明パスゼロ）＝LLM は太い線を**過剰適用せず使い分ける**。

**核の発見（正直版・アドバーサリアル・レビューで2つの major を是正してから）**:
- **honesty gap は消えず“移動”した**＝**線の幅↔射程の再帰**。OR は候補が爆発し（`fire∪flying`=255）、`MAX_DETAIL=60` cap が**名前アルファベット順 slice を total で並べ直す**ため「最強」表示が母集団でなく前綴りの一部になる（rayquaza 780 が落ちる）。**測定された大声の失敗（OR→AND 反転）を、未測定の静かな失敗（アルファベット前綴り上の上位N）に置換**しただけになりかけた。
- **入力役割の忠実化は“入口（意図符号化）”だけでなく“出口（結果提示の誠実さ）”まで通さないと新しいサイレント縮約を生む**。初版はその開示（`matchedCount`・truncation note）を §12 の live『探す』パスに**配線していなかった**（`page.tsx` が書き戻さず・結果リージョンがバインドせず＝開示が dead path）。レビュー指摘で **`/findMons/matchedCount`（候補）と `/findMons/note`（注意）を live パスに繋ぎ**、結果リージョンに必ず置くよう改訂 → 実機で「該当24／**候補255**／『上位60件のみ評価・全体ランキングを取りこぼし得る』」が見えるように。**真の射程（全 union を種族値ランクしてから cap）は全件 fetch のコスト**で、それは払わず「透明な部分評価」を選んだ＝exp02『線の幅↔コスト/レイテンシ』のトレードオフを明示で受ける。
- **§13 の閉ループ教訓がここでも再帰**＝再測ハーネス（`eval-collect.mts`）の badPath ホワイトリストが stale で、§14b が正しく出す新パス（`/shelf/typeMode`・`genFrom`・`genTo`）を機械的契約違反に誤判定していた（＝修正の狙った出力を penalize）。**測定層は「修正が変える要素（新パス・出口の開示）」を観測できる粒度に更新してから測る**。

> §14b の結論: 線（＝サーバのクエリ言語）を実際に太くすると、§13/§14 の **OR/範囲のサイレント縮約は“源から”解消**でき、LLM はその表現力を**過剰適用せず忠実に使い分ける**。だが**忠実化は入口（意図の符号化）だけでは半分**で、**出口（結果提示）の誠実さ＝「上位Nが母集団の何%か」を live パスで開示**しないと、消したはずのサイレント縮約が結果側に再発する（線の幅↔射程の再帰・真の射程は全件コスト）。入力役割の「黙って近似しない」は、入力と出力の**両端**で守って初めて成立する。
