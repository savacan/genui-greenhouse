...
## 15. がっつり監査 — 違和感の正体「フォーム汚染」と総点検（実機＋8サーフェス ground truth・2026-06-26）

ユーザーが手で触って「ロジックがぶっ壊れてる気がする・違和感がすごい」と言った。§14b のランキング修正で味を占めず、**8サーフェスを実 PokéAPI と数値で突き合わせる監査**をワークフローで回した（ランキング/種族値/フォーム/世代/型集合/しきい値/配線/コード）。

**まず検証で固まった健全な面**（ground truth と完全一致）: ランキング数式（集合演算→全候補fetch→stats合計→minStatsフィルタ→sortBy→limit）は water(192)/grass/OR3/各 sortBy で真の上位と1件もズレなし＝§14b の cap バグは確実に解消。種族値の stat マッピングと total も8体で完全一致（spAtk/spDef 取り違えなし）。集合演算・dedup・空状態・minStats 境界・form→state→`/api/find` 配線も数値で検証クリーン。**＝計算は健全だった。**

**違和感の正体＝フォーム汚染（データ設計の問題・計算バグではない）**: `type/<t>.pokemon` は**別形態を全部含む**（dragon は117件中57件が `-mega`/`-gmax`/`-primal`/`-origin`/`-ultra`/`-eternamax`、全て id≥10000）。findMons がそれを de-dup せず候補にし、`sortBy=total` 固定で並べるため、**種族値が異常に高い異形がボード上段を独占**した（dragon 1位 = `eternatus-eternamax` total **1125**＝レイドボス形態で「相棒」ではない・top24 の15/24が別形態。water 1位 kyogre-primal・fire 1位 groudon-primal）。**「データに忠実すぎる」**のが正体。決定的傍証＝**世代フィルタを掛けた瞬間に汚染が消える非対称**（gen 指定時だけ `generation.pokemon_species`＝種名との積でフォームが副作用的に落ちる）＝「タイプだけ汚染・世代も指定で綺麗」という UX 一貫性破綻。

**その中に隠れていた実バグ（major）**: 世代フィルタが `type/<t>`（フォーム名）と `generation/<id>`（**種名**）を生文字列で交差していたため、形態名しか type 側に無い真のドラゴン種（`giratina-altered`=ghost/dragon 680・`zygarde-50`・`tatsugiri-curly`）が**世代指定で誤って全消滅**していた（実測 dragon∩gen4 に giratina 不在）。**従因**＝`sortBy` が UI/route で一切生成されず常に total 固定＝「素早さ高め」でも最速 garchomp-mega-z(151) が埋もれ、遅い巨大形態が頂点に立つ。

**修正（縦スライス・共通基盤なし）**:
- **フォーム正規化＝既定 base 種のみ**: 候補の詳細を fetch 後、`is_default` で base 種だけ残す（メガ等は id≥10000・is_default=false で落ちる）。LLM が `Toggle`「メガ・特殊形態も含める」（`/shelf/includeForms`・既定 false）を compose し、見たい人は ON。＝GenUI らしく粒度を選択肢として持たせる。
- **世代フィルタを `species.name` で**（fetch 後に突き合わせ）→ giratina 等が正しく残り、gen 指定有無で挙動が揃う（フォーム正規化と同じ経路で同時解消）。
- **`sortBy` を配線**: LLM が `並べ替え` Select を compose し問いの強調軸（「素早さ高め」→speed）を初期値に。
- 軽微: `/api/find` の params.parse を try 内へ＋types を3に cap（4タイプ500の解消）・世代ラベルの min/max 正規化と「N以降/Mまで」開放端・Select 値の Number 正規化。デッドコード削除（`idFromUrl`・generate の `intent="find"` 分岐・`buildResultsPrompt`→§12 の単一 live 経路に統一）。
- 検証（ground truth・恒久化）: probe ⑫ を「真の全件ランク（base/forms 別）と突き合わせ取りこぼし0か」に拡張＝両方 PASS。実機で dragon 既定→eternamax/メガ消滅・実在の相棒／dragon gen4→giratina 生存／includeForms ON→別形態復活／sortBy=speed→dragapult 1位／「素早さが高いドラゴン」を LLM が組む→speed Select＋下限110＋トグル OFF→探すで速い実在ドラゴンが速度順（別形態ゼロ）。

> §15 の結論（最大の教訓）: **計算は何度検証しても、「出てきた“もの”が妥当か」を見なければ破綻は見つからない**。eternamax が常に1位だったのは Phase A からずっとで、件数・機構・stat 値の検証は全部通っていた——**ユーザーが手で触るまで誰も見なかった**。「違和感」は一級の信号。そして破綻は計算バグでなく **データの“粒度”（フォーム名 vs 種名）という設計判断**だった＝exp02「線の幅↔射程」の線は、クエリ言語だけでなく**母集団の粒度**でもある。題材データは「取ってそのまま並べる」と必ずこの粒度問題を持つ（実 API は人間の「相棒」概念より細かい/雑多）。GenUI でも**ドメインの正しい粒度に正規化する責務はサーバ側に残る**。

## 16. エンドツーエンド意味検証 — 「返ってきたモノが問い通りか」を測定にする（14クエリ・ground truth・2026-06-27）

§15 の教訓（出力の妥当性を見る）を**反復可能な測定**にした。§11 は「組まれた*フォーム*が問いを符号化したか」までだったが、ここは **クエリ → LLM が compose → 実際の `toFindParams` → `/api/find` → 返ってきたポケモン**まで通し、「**その相棒が日本語の問いを実際に満たすか**」を実 PokéAPI を ground truth に多レンズ判定（`scripts/eval-e2e.mts` 収集 → ワークフロー14判定）。`toFindParams` は `lib/finder/shelf.ts` に切り出して page と eval が共有＝**検証が“実際の変換”を測る**（複製ドリフトで嘘にならないように）。

**結果（scoreAvg 3.79 / 14中9が完全忠実）**: 実験の核「LLM は双方向フォームを組めるか」は **YES で確定**。§11 で最大破綻だった **OR→AND サイレント意図反転は完全に克服**（「炎か飛行」「エスパーか鋼」が typeMode=or で literal に union を満たす）。AND・世代範囲・minStats・sortBy・includeForms の符号化も妥当。非表現語の graceful 明示（§13）も機能（「一番強い」→total に再定義し明示／「かわいい」「伝説級」→fabricate せず明示）。**＝残る破綻は LLM 側ではない。**

**この検証が初めて見つけた破綻（構造化監査 §15 は見逃した・全てサーバ側）**:
- **🔴 blocker＝1件の 404 で検索全体がクラッシュ**。「エスパーか鋼」の type union に `type/psychic` 由来の幽霊フォーム名 `meowstic-male-mega` が入り、`/pokemon/meowstic-male-mega` が 404 → `mapLimit` の `Promise.all` が reject → `/api/find` ごと死亡。本来返る gholdengo(spAtk133)/munkidori/archaludon… 7件が丸ごと消えた。§15 監査は「404 ゼロ」と報告していた（サンプリングの穴）＝**全クエリを実際に通して初めて出た**。修正＝候補 fetch を per-item try/catch で 404→null→除外。
- **🟠 false-disclosure＝「タイプ未指定でも探せます」の嘘**。「壁ポケモン」等の type なしクエリで、サーバは `types.min(1)` 必須なのに LLM が type を optional として「未指定でも探せる」と案内＝§13/CLAUDE.md が禁じる“できない約束”。修正＝compose プロンプトに `types` 必須を明記し「種族値だけでは検索できません。タイプを選んでください」と促す（嘘を撲滅）。
- **🟠 能力述語の偽装＝「メガシンカできる」を includeForms にすり替え**。findMons に「メガ進化可否」フィルタは無いのに includeForms=true で満たしたかに見せ、しかも includeForms は「別形態を*表示*」であって「メガ可能種を*絞る*」ではない（別物）。修正＝能力/メタ述語（メガ可否・伝説級・進化前）は中立化＋「条件として絞れません」と明示し、includeForms を proxy にしない。
- **データソース汚染（data-decision）**: PokéAPI は**非公式（fan-made）メガ**を form として保持（`heatran-mega`・`delphox-mega` 等が 200 で返る）＝「PokéAPI＝公式」の実在性前提が崩れる。docs に記録（メガ可否を真に判定するなら公式ホワイトリストが要る）。

**修正と再測（同14クエリ・回帰チェック）**: 404 クラッシュ→gholdengo 等7件が復活／「壁」→steel の probopass 等が出る（検索なし解消）／「メガできる」→includeForms=false・明示／「タフ」3軸 AND で0件→1軸 sort で非空に／既存9クエリは無回帰。エラー経路の内部 URL 漏れも中立文に正規化。0件 note も原因名指し（下限が高すぎ／別形態を含めると有り）に改善。

> §16 の結論: **「出力の妥当性を見る」は一度きりの気づきでなく、反復可能な測定にできる**——クエリ→フォーム→検索→結果を通し「返ってきた相棒が問い通りか」を ground truth で判定する eval は、構造化監査が見逃した blocker（404 クラッシュ）を一発で出した。そして測定が確定したこと＝**入力役割（フォーム compose）の品質は load-bearing に高い**（OR 反転も克服）一方、**「毎回確実に」を阻むのは一貫してサーバ側**（クエリ言語の表現力＝メガ可否/legendary・実装堅牢性＝404 耐性・データソース＝非カノン）＝exp02「線の幅↔射程」と同型で、GenUI の伸びしろは LLM でなく**サーバの射程と堅牢性**にある。
