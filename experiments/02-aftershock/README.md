...
- **再フェッチ税の除去 ✅**（§12・`priorEntities`）— 前ターンの**スカラーだけ**（eventId/lat/lon）を越境させ、参照フォローアップの再取得を省く。turn2「その震源の周りは？」が 6手→**2手**・totalIn 10.8k→**3.0k**・~62s→**12.4s**。firewall は維持（生配列は越境させない）。§8(a) と同型のトレードがターン境界にも。

**この実験を貫く1本の軸 = 「スカラーは通す・生配列は通さない」**。ループ内（§8）・spec 経路（§9）・濃さの上限（§11）・ターン境界（§12）の全てで同じ線引きが効き、動かすのは常に「線の幅 ↔ 射程／コスト／レイテンシ」だった。
