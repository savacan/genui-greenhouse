...
/** 1アクション分の hint（複数アクション = 複合質問のときは複数渡る）。 */
...
/**
 * Step 2 のユーザープロンプト = データ→プロンプトのファイアウォール。
 * 生データや配列は一切渡さず、$state パス・型・意味・件数だけを渡す。
 * 複数アクション（複合質問）は全部を1画面に組ませる。
 */
export function buildComposePrompt(query: string, parts: ComposePart[]): string {
  ...
  return [
    `ユーザーの問い: "${query}"`,
    multi
      ? `この問いは複数トピックに跨る。下の全アクションのデータを1画面に組む（セクションを分け、見出しで区切る）。`
    ...
    followups.length
      ? [
          ...
          `各 ActionButton には必ず on を付ける: "on": {"click": {"action": "ask", "params": {"query": "<問い>"}}}。label は短く（query と同じでなくてよい）。`,
    ...
    [
      ...
      `  桁区切りは options:{"useGrouping":true}。これは Kpi.value や Text に直接出す数値にだけ使う。`,
      `  生の配列/数値を受ける部品（IssMap / AsteroidTable / AsteroidScatter / OrbitProximity(points) / ScatterPlot / Histogram / LaunchTimeline / Gallery(images) / List(items) / SolarWindGauges(series) / KpForecastStrip(bars) / SunEarthLane / AuroraOvalGlobe(band) / FlareEventRail(items)）に渡す値は生のまま（$state だけ）。整形すると壊れる。`,
    ].join("\n"),
