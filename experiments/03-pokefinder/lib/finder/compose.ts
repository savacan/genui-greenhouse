...
export function buildFormPrompt(query: string, types: TypeVocab[], generations: GenVocab[], seed?: SeedMon | null): string {
  ...
  return [
    ...
    `- フォームの下に「結果」リージョンを同じ画面に置く（探すと同じ画面で結果が live 更新される）: Heading「結果」＋ Kpi(label「該当」, value に {"$state":"/findMons/count"}) ＋ Kpi(label「候補」, value に {"$state":"/findMons/matchedCount"}, unit「件」) ＋ Text(muted, text に {"$template":"条件: \${/findMons/criteriaLabel}"}) ＋ Text(muted, text に {"$state":"/findMons/note"})（候補が多く OR 等で上位だけ評価したときの注意がここに出る・空なら何も出ない）＋ MonGrid(mons に {"$state":"/findMons/mons"})。/findMons は最初は空（MonGrid は空状態を出す）。「探す」を押すとサーバが値（該当・候補・注意・mons）を入れて更新される。**「候補」と注意は必ず置く**（OR や広いタイプは候補が cap を超え上位N件しか評価しない＝該当だけ出すと取りこぼしが不可視になる）。`,
    ``,
